...
qplot(Unemployment$unemp, colour=Unemployment$year, data=Unemployment, geom="density")
...
# Obviously there is actually only one factor here which is time, so two way annova won't give desired results, one way annova with year is sufficient


Unemployment_lm <- lm(Unemployment$unemp ~ Unemployment$year, data = Unemployment)
Unemployment_lm_summary = summary(Unemployment_lm)
Unemployment_lm_summary
if (Unemployment_lm_summary$r.squared < 0.5)
{
...
Unemployment_aov <- aov(Unemployment_lm)
Unemployment_aov_summary = summary(Unemployment_aov)
Unemployment_aov_summary
